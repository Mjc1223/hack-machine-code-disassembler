            ...
            if (File.Exists(args[0]))
            {
               ...
               foreach (string line in Lines)
               {
                  ...
                  if(line[0] == '0') {
                     ...
                     string aInstruction = "@" + val;
                     hackList.Add(aInstruction);

                  }
                  else {
                     string cBit, dBit, jBit;
                     string cInstruction;

                     cBit = line.Substring(3,7);
                     dBit = line.Substring(10,3);
                     jBit = line.Substring(13,3);

                     cBit = compTable[cBit];
                     dBit = destTable[dBit];
                     jBit = jumpTable[jBit];

                     cInstruction = dBit + cBit + jBit;
                     hackList.Add(cInstruction);
                  }
